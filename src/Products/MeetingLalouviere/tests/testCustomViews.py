...
class testCustomViews(MeetingLalouviereTestCase):
    ...
    def setUp(self):
        ...
        commission_categories = []  # flatten COUNCIL_MEETING_COMMISSION_IDS_2019
        for i in COUNCIL_MEETING_COMMISSION_IDS_2019:
        ...
        self.meeting = self.create("Meeting", date="2019/09/1 18:00:00")
        ...
        view = self.meeting.restrictedTraverse("@@document-generation")
        view()
        self.helper = view.get_generation_context_helper()

    def test_get_commission_items(self):
        item = self.create("MeetingItem")
        item.setCategory(COUNCIL_MEETING_COMMISSION_IDS_2019[0][0])  # First commission
        item2 = self.create("MeetingItem")
        item2.setCategory(
            COUNCIL_MEETING_COMMISSION_IDS_2019[1][0]
        )  # Second commission
        ...
        item_s.setCategory(
            COUNCIL_MEETING_COMMISSION_IDS_2019[0][0] + "-1er-supplement"
        )  # First commission, 1st supp
        item2_s = self.create("MeetingItem")
        item2_s.setCategory(
            COUNCIL_MEETING_COMMISSION_IDS_2019[1][0] + "-1er-supplement"
        ...
        comm_no1_items = self.helper.get_commission_items(item_uids, 1)
        self.assertEquals(comm_no1_items[0], item)
        self.assertEquals(len(comm_no1_items), 1)

        comm_no2_items = self.helper.get_commission_items(item_uids, 2)
        self.assertEquals(comm_no2_items[0], item2)
        self.assertEquals(len(comm_no2_items), 2)

        comm_no3_items = self.helper.get_commission_items(item_uids, 3)
        self.assertEquals(len(comm_no3_items), 0)

        # Tests cases with type='supp'
        comm_no1_items = self.helper.get_commission_items(
            item_uids, 1, type="supplement"
        )
        self.assertEquals(comm_no1_items[0], item_s)
        self.assertEquals(len(comm_no1_items), 1)

        comm_no2_items = self.helper.get_commission_items(
            item_uids, 2, type="supplement"
        )
        self.assertEquals(comm_no2_items[0], item2_s)
        self.assertEquals(len(comm_no2_items), 2)

        comm_no3_items = self.helper.get_commission_items(
            item_uids, 3, type="supplement"
        )
        self.assertEquals(len(comm_no3_items), 0)

        # Tests cases with type='*', get all items for each commission regardless of type
        comm_no1_items = self.helper.get_commission_items(item_uids, 1, type="*")
        self.assertEquals(comm_no1_items[0], item)
        self.assertEquals(len(comm_no1_items), 2)

        comm_no2_items = self.helper.get_commission_items(item_uids, 2, type="*")
        self.assertEquals(comm_no2_items[2], item2_s)
        self.assertEquals(len(comm_no2_items), 4)

        comm_no3_items = self.helper.get_commission_items(item_uids, 3, type="*")
        self.assertEquals(len(comm_no3_items), 0)

    def test_format_commission_pre_meeting_date(self):
        self.meeting.setPreMeetingDate(DateTime(2019, 9, 1, 4, 20, 0))
        self.meeting.setPreMeetingPlace("Pays des bisounours")
        self.meeting.setPreMeetingDate_2(DateTime(2019, 9, 2, 4, 20, 0))
        self.meeting.setPreMeetingPlace_2("Sur la Lune")

        self.assertEquals(
            self.helper.format_commission_pre_meeting_date(1),
            "(Sunday 01 september 2019 (04H20), Pays des bisounours)",
        )
        self.assertEquals(
            self.helper.format_commission_pre_meeting_date(2),
            "(Monday 02 september 2019 (04H20), Sur la Lune)",
        )

    def test_get_commission_assembly(self):
        self.meeting.setPreMeetingAssembly("myAssembly")
        self.meeting.setPreMeetingAssembly_2("myAssembly 2")
        self.assertEqual(
            self.meeting.getPreMeetingAssembly(), self.helper.get_commission_assembly(1)
        )
        self.assertEqual(
            self.meeting.getPreMeetingAssembly_2(),
            self.helper.get_commission_assembly(2),
        )
