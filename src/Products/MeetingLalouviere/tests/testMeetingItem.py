...
class testMeetingItem(MeetingLalouviereTestCase, mctmi):
    ...
    def test_pm_PowerObserversLocalRoles(self):
        ...
        self.meetingConfig.setItemPowerObserversStates(
            (
                "itemcreated",
                "validated",
                "presented",
        ...
        self.meetingConfig.setMeetingPowerObserversStates(
            ("created", "frozen", "decided", "closed")
        )
        self.meetingConfig.setItemRestrictedPowerObserversStates(
            ("itemfrozen", "accepted", "refused")
        )
        self.meetingConfig.setMeetingRestrictedPowerObserversStates(
    ...
    def test_pm_OnTransitionFieldTransforms(self):
        ...
        self.assertTrue(
            duplicatedItem.getDecision(keepWithNext=False) == originalDecision
        )
        ...
        self.assertTrue(
            item4.getDecision(keepWithNext=False)
            == "<p>My decision that will not be touched.</p>"
        )
    ...
    def test_pm_IsPrivacyViewable(self):
        ...
        secretItemPloneGroupsOfProposingGroup = getattr(
            self.tool, secretItem.getProposingGroup()
        ).getPloneGroups(idsOnly=True)
        self.failIf(
            set(secretItemPloneGroupsOfProposingGroup).intersection(
